...
<p style="font-size:12px;font-weight:700;letter-spacing:.06em;text-transform:uppercase;color:#e94560;margin:0 0 8px;">Payment not verified</p>
<h1 style="font-size:20px;margin:0 0 4px;">{{ $event->title }}</h1>
<p style="font-size:13px;color:#5b5b6b;margin:0;">Hi {{ $registration->name }}, the organizer couldn't verify your payment reference. Your QR pass hasn't been issued.</p>
</td></tr>
<tr><td style="padding:0 28px 8px;">
<p style="font-size:12px;background:#fbeaea;color:#b3243c;border-radius:8px;padding:10px 12px;margin:0;">Double-check your reference number and screenshot, then contact the organizer{{ $event->organization?->email ? ' at '.$event->organization->email : '' }} to resolve this.</p>
</td></tr>
...
<p style="margin:0 0 6px;"><strong>Reference submitted:</strong> {{ $registration->payment_ref }}</p>
<p style="margin:0;"><strong>Event date:</strong> {{ \Carbon\Carbon::parse($event->date)->format('l, F j, Y') }}</p>
</td></tr>
</table>
</td></tr>
</table>
